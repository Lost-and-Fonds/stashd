...
| New broadcast workflow (media-server) | Identify connection-valued Broadcast settings and list compatible configured Connections | `GET /api/v1/connections` exists, but `/broadcast-plugins` does not expose a plugin's `connection_setting_key`; Plex therefore arrives as a required text field | Expose generic resource-field metadata, then add a Stashd-owned connection picker in the later Connections slice |
