...
| New stash workflow | Render an explicit Input-plugin picker, its declared source-acquisition fields, and the read-only generic preflight result | Backend exposes `/api/v1/input-plugins` and plugin-scoped preflight; UI-v2 still simulates URL detection | Build the New Stash slice without provider branches |
| New stash workflow | Create-Stash + initial Input as one action/transaction | Still separate writes: create Stash, then add the resolved Input | Design an atomic create endpoint when the live New Stash workflow is implemented |
