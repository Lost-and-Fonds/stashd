...
jellyfin_refresh_after=$(request_count "$TMP/jellyfin/requests.jsonl"); [ "$jellyfin_refresh_after" -gt "$jellyfin_refresh_before" ] || exit 1
touch "$TMP/jellyfin/refresh-fail"; jellyfin_failed_action=$(post_json "$BASE/api/v1/broadcasts/$jellyfin_id/actions" '{"intent":"refresh_library"}'); jellyfin_failed_job_id=$(printf '%s' "$jellyfin_failed_action" | jq -r '.operation.id'); jellyfin_failed_job=''; for _ in $(seq 1 90); do jellyfin_failed_job=$(get_json "$BASE/api/v1/jobs/$jellyfin_failed_job_id"); [ "$(printf '%s' "$jellyfin_failed_job" | jq -r '.job.state')" = failed ] && break; sleep 2; done; printf '%s' "$jellyfin_failed_job" | jq -e '.job.state == "failed" and .job.last_error != null' >/dev/null; rm "$TMP/jellyfin/refresh-fail"
