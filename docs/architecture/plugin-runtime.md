# Stashd plugin runtime boundary

M7.5 moved the proven plugin behavior out of the milestone spikes and
into deliberate package boundaries. This document records the production
plugin boundary after the M8/M9 provider ports.
...
The following are stable enough for the Jellyfin plugin port:
...
Wasmtime is not a production runtime. Plugin is the only production runtime;
