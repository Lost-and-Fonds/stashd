...
check fresh-schema boot and the relevant upgrade path against PostgreSQL during build/release work.
...
`2026_07_15_drop_stash_slug`). The baseline and released post-baseline migrations define the supported upgrade
path. Older migration implementation is Git history, not a runtime dependency.
