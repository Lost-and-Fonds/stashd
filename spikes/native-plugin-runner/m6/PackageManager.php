...
namespace Stashd\NativePackages;
...
{
    /** @param list<string> $extensions @param list<string> $architectures */
    ...
    public static function fromFile(string $path, string $apiVersion = '0.1', ?string $architecture = null): self
    {
        ...
        if ($architectures !== [] && $architecture !== null && !in_array($architecture, $architectures, true)) {
        ...
        return new self($data['id'], $data['name'], $data['version'], $data['runtime'], $data['api_version'], $data['entrypoint'], $phpConstraint, $extensions, $architectures);
    }
...
final class PackageManager
{
    ...
    public function rollback(string $id, string $version): void { $this->activate($id, $version); }
    ...
    public function link(string $id, string $source): PackageManifest
    {
        ...
        $this->disable($id);
        $link = $this->links . '/' . $id;
        if (is_link($link) || file_exists($link)) { $this->removeTree($link); }
        if (!symlink($source, $link)) { throw new PackageStateError('development link could not be created'); }
    ...
    public function activeVersion(string $id): ?string
    {
        ...
        try { return PackageManifest::fromFile($manifest, $this->apiVersion, $this->architecture ?? self::architecture())->version; } catch (PackageValidationError) { return null; }
    }
    ...
    private function validateId(string $id): void { if (preg_match('/^[a-z][a-z0-9-]{1,63}$/', $id) !== 1) { throw new PackageStateError('plugin ID is invalid'); } }
    private function validateVersion(string $version): void { if (preg_match('/^\d+\.\d+\.\d+(?:[-+][0-9A-Za-z.-]+)?$/', $version) !== 1) { throw new PackageStateError('plugin version is invalid'); } }
    private static function architecture(): string { return match (php_uname('m')) { 'x86_64', 'amd64' => 'amd64', 'aarch64', 'arm64' => 'arm64', default => php_uname('m') }; }
    private function makeImmutable(string $path): void { $this->walkMode($path, 0444, 0555); }
    private function makeMutable(string $path): void { $this->walkMode($path, 0644, 0755); }
    private function walkMode(string $path, int $fileMode, int $directoryMode): void
    {
        if (is_link($path)) { return; }
        if (is_dir($path)) { chmod($path, $directoryMode); foreach (scandir($path) ?: [] as $entry) { if ($entry !== '.' && $entry !== '..') { $this->walkMode($path . '/' . $entry, $fileMode, $directoryMode); } } return; }
        chmod($path, $fileMode);
    }
...
final class TarArchive
{
    public static function extract(string $archive, string $destination): void
    {
        ...
        try {
            while (true) {
                ...
                if ($header === '' || strlen($header) < 512) { throw new PackageValidationError('archive is truncated'); }
                ...
                if ($path === '' || isset($seen[$path])) { throw new PackageValidationError('archive contains a duplicate or empty path'); }
                $seen[$path] = true;
                self::validatePath($path);
                if ($type === '1' || $type === '2') { throw new PackageValidationError('archive links are not permitted'); }
                if ($type !== "\0" && $type !== '0' && $type !== '5') { throw new PackageValidationError('archive entry type is unsupported'); }
                $target = $destination . '/' . $path;
                if ($type === '5') {
                    if (!mkdir($target, 0700, true) && !is_dir($target)) { throw new PackageValidationError('archive directory could not be extracted'); }
                } else {
                    $parent = dirname($target);
                    if (!@mkdir($parent, 0700, true) && !is_dir($parent)) { throw new PackageValidationError('archive parent could not be created'); }
                    $output = fopen($target, 'xb');
                    if ($output === false) { throw new PackageValidationError('archive file could not be created'); }
                    self::copy($stream, $output, $size);
                    fclose($output);
                }
                self::skipPadding($stream, $size);
            }
        } finally { self::close($stream); }
    }

    private static function open(string $archive): mixed
    {
        $stream = str_ends_with($archive, '.gz') ? gzopen($archive, 'rb') : fopen($archive, 'rb');
        if ($stream === false) { throw new PackageValidationError('archive could not be opened'); }
        return $stream;
    }
    private static function read(mixed $stream, int $length): string { $value = is_resource($stream) && get_resource_type($stream) === 'stream' ? fread($stream, $length) : gzread($stream, $length); return $value === false ? '' : $value; }
    private static function copy(mixed $input, mixed $output, int $size): void
    {
        $remaining = $size;
        while ($remaining > 0) { $chunk = self::read($input, min(8192, $remaining)); if ($chunk === '') { throw new PackageValidationError('archive file is truncated'); } fwrite($output, $chunk); $remaining -= strlen($chunk); }
    }
    private static function skipPadding(mixed $stream, int $size): void { $padding = (512 - ($size % 512)) % 512; if ($padding > 0) { self::read($stream, $padding); } }
    private static function close(mixed $stream): void { if (is_resource($stream)) { get_resource_type($stream) === 'stream' ? fclose($stream) : gzclose($stream); } }
    ...
    private static function validatePath(string $path): void { $parts = explode('/', $path); if (str_starts_with($path, '/') || in_array('', $parts, true) || in_array('..', $parts, true) || in_array('.', $parts, true)) { throw new PackageValidationError('archive path is unsafe'); } }
    private static function octal(string $value): int { return octdec(trim($value, "\0 ")) ?: 0; }
}
