...
final class M7Rpc
{
    ...
    public function __construct() { $this->hello(); }
    ...
    public function call(string $method, array $params): array
    {
        ...
        while (true) {
            ...
            if (($message['kind'] ?? null) === 'notification') { continue; }
            if (($message['id'] ?? null) !== $id) { throw new RuntimeException('RPC response ID mismatch'); }
            if (isset($message['error'])) { throw new RuntimeException((string) ($message['error']['message'] ?? 'host capability failed')); }
    ...
    public function hello(): void
    {
        ...
        if (($response['id'] ?? null) !== $id || ($response['result']['protocol'] ?? null) !== 1) { throw new RuntimeException('RPC handshake failed'); }
    }
}
...
final class M7Resource implements ReadableResource
{
    ...
    public function isEof(): bool { return $this->eof; }
    public function close(): void { $this->eof = true; }
}
...
final class M7Log implements \Stashd\PluginSdk\Logger
{
    public function __construct(private M7Rpc $rpc) {}
    public function info(string $message): void { $this->rpc->call('event.log', ['message' => $message]); }
    public function error(string $message): void { $this->rpc->call('event.log', ['message' => $message, 'level' => 'error']); }
}

final class M7Progress implements \Stashd\PluginSdk\ProgressReporter
{
    public function __construct(private M7Rpc $rpc) {}
    public function report(string $stage): void { $this->rpc->call('event.progress', ['stage' => $stage]); }
}
...
final class M7ExampleBroadcast implements BroadcastPlugin
{
    ...
    public function prepare(PublishRequest $request): Preparation { $this->context->progress->report('prepare'); return new Preparation(); }
    public function publish(PublishRequest $request): Publication
    {
        ...
        if ($small->body() !== 'small-response') { throw new RuntimeException('small broker response mismatch'); }
        $large = $this->context->http->request('GET', 'https://allowed.test/large');
        $bytes = 0;
        if ($large->resource === null) { throw new RuntimeException('large broker response was not a resource'); }
        while (!$large->resource->isEof()) { $bytes += strlen($large->resource->read(8192)); }
        if ($bytes !== 300_000) { throw new RuntimeException('large broker resource mismatch'); }
        $asset = $this->rpc->call('asset.read', ['reference' => 'asset-1']);
        if (base64_decode((string) $asset['data'], true) !== 'asset-content') { throw new RuntimeException('asset handle mismatch'); }
        $helper = $this->rpc->call('helper.run', ['name' => 'fixture-helper', 'arguments' => []]);
        if ((int) $helper['exit_code'] !== 0) { throw new RuntimeException('helper failed'); }
    ...
    public function finalize(FinalizationRequest $request): Publication { $this->context->progress->report('finalize'); return $request->publication; }
    public function operation(OperationRequest $request): OperationResult { return new OperationResult([new Choice('fixture', 'Fixture choice')], [new Setting('echo', OptionValue::text($request->name))]); }
}

final class M7ExampleInput implements InputPlugin
{
    public function __construct(private PluginContext $context) {}
    public function resolve(string $source): ResolvedInput { return new ResolvedInput('input-1', 'fixture:' . $source, 'fixture', 'Fixture input'); }
    public function discover(string $inputId, DiscoveryIntent $intent, array $options = []): array { return [new DiscoveredItem('item-1', 'fixture:item-1', 'Fixture item', kind: 'binary')]; }
