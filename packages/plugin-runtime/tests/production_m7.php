...
require_once __DIR__ . '/../src/Package/OciRegistry.php';
...
require_once __DIR__ . '/../src/Runner/PluginProcess.php';
require_once __DIR__ . '/../src/Runner/PluginRunner.php';
...
try {
    ...
    $manifest = $manager->installOciLayout($built['layout'], $built['digest'], 'example.test/m7-example:1.0.0');
    $manager->activate($manifest->id, $manifest->version);
    m7Assert($manager->installOciLayout($built['layout'], $built['digest'])->id === 'm7-example', 'same digest install was not idempotent');
    m7Assert(($manager->installed()[0]['digest'] ?? null) === $built['digest'], 'installed digest was not recorded');
