...
final readonly class PluginInputDefinition
{
    ...
    {
        if (($manifest['kind'] ?? null) !== 'input' || ! is_string($manifest['id'] ?? null)) return null;
        $options = [];
        foreach (is_array($manifest['input_options'] ?? null) ? $manifest['input_options'] : [] as $raw) {
            if (! is_array($raw) || ! is_string($raw['key'] ?? null) || ! is_string($raw['label'] ?? null)) continue;
            $type = InputOptionType::tryFrom((string) ($raw['type'] ?? ''));
            if ($type === null || ! is_bool($raw['default'] ?? null) && ! is_string($raw['default'] ?? null)) continue;
        ...
        if (is_array($manifest['helper'] ?? null) && is_string($manifest['helper']['name'] ?? null) && is_string($manifest['helper']['executable'] ?? null)) {
            ...
            if (is_file($path)) $helper = new PluginHelperGrant($manifest['helper']['name'], $path, $root);
        }
    ...
    public function httpGrants(SecretsService $secrets, string $operation): array
    {
        ...
        foreach ($this->grants as $raw) {
            ...
            if ($ops !== [] && ! in_array($operation, $ops, true)) continue;
            ...
            if (is_array($raw['credential'] ?? null)) {
                $c = $raw['credential']; $value = is_string($c['secret_key'] ?? null) ? $secrets->get($c['secret_key']) : null;
                if ($value !== null && $value !== '') $credential = new PluginCredentialGrant((string) ($c['name'] ?? ''), $value, (string) ($c['parameter'] ?? 'key'), (string) ($c['placement'] ?? 'query'));
            }
