...
final readonly class ActivityEventService
{
    ...
    {
        return $this->emit(ActivityLevel::Success, 'stash.input_added', sprintf('Input added to stash with %d new items.', $result->itemsCreated), 'stash', $result->stashId, stashId: $result->stashId, jobId: (string) $job->id, groupKey: 'job:' . (string) $job->id, metadata: $result->toArray());
    }
    ...
    public function downloadFailed(JobRecord $job, string $code, string $error): ActivityEventRecord
    {
        ...
        $itemId = is_string($rawItemId) ? $rawItemId : null;
        $rawStashId = $payload['stash_id'] ?? null;
        $stashId = is_string($rawStashId) ? $rawStashId : null;

        return $this->emit(ActivityLevel::Error, 'download.failed', $this->secrets->redact($error), 'job', (string) $job->id, stashId: $stashId, itemId: $itemId, jobId: (string) $job->id, groupKey: 'job:' . (string) $job->id, metadata: ['code' => $code]);
    }

    /** @param array<string, mixed>|null $metadata */
    private function emit(ActivityLevel $level, string $type, string $message, ?string $entityType = null, ?string $entityId = null, ?string $stashId = null, ?string $itemId = null, ?string $broadcastId = null, ?string $jobId = null, ?string $groupKey = null, ?array $metadata = null): ActivityEventRecord
    {
        $record = $this->events->create(level: $level, type: $type, message: $message, entityType: $entityType, entityId: $entityId, stashId: $stashId, itemId: $itemId, broadcastId: $broadcastId, jobId: $jobId, groupKey: $groupKey, metadata: $metadata);
